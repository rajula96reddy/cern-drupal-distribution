 ...
 */
class WebcastFetcherFeedForm extends ExternalPluginFormBase implements ContainerInjectionInterface {
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state, FeedInterface $feed = NULL) {
    $form['categories'] = array(
      ...
      '#description' => $this->t('Category ID(s) to retrieve events from. Separate multiple IDs with commas. To find out the ID of a category, go to the category in <a href="http://webcast.cern.ch">Webcast</a> and copy the numeric value next to <em>/category/</em> from the URL.'),
      '#required' => FALSE,
      '#maxlength' => 1024,
    );
  ...
  public function validateConfigurationForm(array &$form, FormStateInterface $form_state, FeedInterface $feed = NULL) {
    ...
    if (!$values['categories'] && !$values['date_from'] && !$values['date_to']) {
